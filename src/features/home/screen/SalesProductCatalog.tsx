...
import { addToCart, decreaseQuantity, increaseQuantity, removeFromCart } from "../../cart/store/cartSlice";
...
export const SalesProductCatalog: React.FC<SalesProductCatalogProps> = ({
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      setError(null);
      // استدعي الـ API الخاص بالمنتجات هنا
      const mockProducts: ProductDto[] = [
        { id: "1", name: "منتج أ - عبوة كبيرة", categoryName: "مشروبات", unitSellingPrice: 15.0, availableStock: 50 },
        { id: "2", name: "منتج ب - كرتونة صغيرة", categoryName: "أغذية", unitSellingPrice: 45.5, availableStock: 12 },
        { id: "3", name: "منتج ج - طرد ممتاز", categoryName: "منظفات", unitSellingPrice: 100.0, availableStock: 0 },
      ];
  ...
  return (
    <div className="p-6 bg-brand-bg min-h-screen text-brand-text dir-rtl mt-12" dir="rtl">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-brand-text mb-1">كتالوج المنتجات</h1>
          <p className="text-brand-subtext text-sm">استعرض المنتجات المتاحة وأضفها للسلة.</p>
        </div>
        <button
          ...
          return (
            <div
              ...
              className={`bg-brand-card rounded-xl border p-4 shadow-sm flex flex-col justify-between transition-all ${
                inCartQty > 0 ? "border-brand-primary ring-1 ring-brand-primary/20" : "border-slate-200"
              }`}
            >
              <div>
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isOutOfStock ? "bg-rose-50 text-rose-600" : "bg-emerald-50 text-emerald-600"}`}>
                    {isOutOfStock ? "غير متوفر" : `المخزون: ${product.availableStock}`}
                  </span>
                </div>

                <h3 className="font-bold text-sm text-brand-text mb-2 line-clamp-2">{product.name}</h3>
              </div>

              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                <div>
                  <span className="text-xs text-brand-subtext block">السعر</span>
                  <span className="text-base font-extrabold text-brand-primary">${product.unitSellingPrice.toFixed(2)}</span>
                </div>

                {isOutOfStock ? (
                  <button disabled className="px-3 py-1.5 bg-slate-100 text-slate-400 text-xs font-semibold rounded-lg cursor-not-allowed">
                    غير متاح
                  </button>
                ) : inCartQty === 0 ? (
                  <button
                  ...
                  <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-lg">
                    <button
                      onClick={() => dispatch(decreaseQuantity({ productId: product.id }))}
                      className="p-1 bg-white hover:bg-slate-200 rounded text-brand-text transition-colors"
                    >
                      <IoRemoveOutline size={14} />
                    </button>
                    <span className="text-xs font-bold w-5 text-center">{inCartQty}</span>
                    <button
                      onClick={() => dispatch(increaseQuantity({ productId: product.id }))}
        ...
        <div className="fixed bottom-4 left-4 right-4 md:left-8 md:right-8 z-40 bg-slate-900 text-white rounded-2xl p-4 shadow-2xl flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <span className="text-lg font-extrabold text-white">${totalCartAmount.toFixed(2)}</span>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <button onClick={() => setIsCartOpen(true)} className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-semibold">
              عرض السلة ({cart.length})
            </button>
            <button onClick={onProceedToOrder} className="flex items-center gap-2 px-5 py-2.5 bg-brand-primary hover:bg-brand-primary/90 text-white rounded-xl text-xs font-bold">
              متابعة إنشاء الطلب
          ...
          <div className="bg-brand-card w-full max-w-md h-full p-6 flex flex-col justify-between shadow-2xl">
            <div>
              <div className="flex justify-between items-center pb-4 border-b border-slate-100 mb-4">
                ...
                <button onClick={() => setIsCartOpen(false)} className="text-brand-subtext hover:text-brand-text text-xl">✕</button>
              </div>

              <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
                {cart.map((item) => (
                  <div key={item.productId} className="p-3 bg-brand-bg rounded-xl border border-slate-100 flex items-center justify-between text-xs">
                    <div>
                      <h4 className="font-bold text-brand-text mb-1">{item.productName}</h4>
                      <p className="text-brand-subtext">${item.unitSellingPrice.toFixed(2)} × {item.quantity}</p>
                    </div>

                    <div className="flex items-center gap-3">
                      <span className="font-bold text-brand-primary">${(item.unitSellingPrice * item.quantity).toFixed(2)}</span>
                      <button
            ...
            <div className="pt-4 border-t border-slate-100 space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-extrabold text-brand-primary">${totalCartAmount.toFixed(2)}</span>
              </div>
